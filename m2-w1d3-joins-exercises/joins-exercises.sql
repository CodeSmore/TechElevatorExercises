...
WHERE c.name = 'Family' AND f.length < 120 AND f.rating = 'G'
...
-- (2 rows) uuuh, I got more than 2, idk
...
SELECT TOP 10 cust.first_name, cust.last_name, COUNT(r.rental_id)
FROM customer cust
...
-- NEEDS RENTAL RATE FROM FILM TABLE...
...
SELECT s.store_id, a.address, COUNT(r.rental_id) as rentalcount, SUM(p.amount) as totalsales, (SUM(p.amount) / COUNT(r.rental_id)) as averagesale
FROM store s
...
SELECT c.name, COUNT(r.rental_id) as rentalcount
FROM category c
...
-- mostly right...from 2-11...
SELECT TOP 11 a.first_name, a.last_name, COUNT(r.rental_id) as rentalcount
FROM actor a
JOIN film_actor fa ON a.actor_id = fa.actor_id
